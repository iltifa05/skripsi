			...
			<div class="container">
				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Laporan Jadwal Asrama</h4>
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="jadwal">Laporan Jadwal Asrama</a>
							</li>
						</ul>
					</div>
										...
										<div class="row mt-4">
											<div class="col-lg-12 col-md-12">
												<?php if(isset($_GET['submit'])) { ?>
													...
													<?php if(isset($_GET['tipe'])) { ?>
														...
														<?php if($_GET['tipe'] == "pertanggal") { 
															...
															$to = $_GET['to']; 
															?>
															<a href="print/c-jadwal?tipe=pertanggal&from=<?= $from; ?>&to=<?= $to; ?>" target="_blank" class="btn btn-success">
														...
														<?php }else if($_GET['tipe'] == "perbulan") { 
															...
															$tahun = $_GET['tahun'];
															?>
															<a href="print/c-jadwal?tipe=perbulan&bulan=<?= $bulan; ?>&tahun=<?= $tahun; ?>" target="_blank" class="btn btn-success">
																<i class="fas fa-print"></i>
															</a>
														<?php }else if($_GET['tipe'] == "pertahun") { 
															$tahun = $_GET['tahun'];
															?>
															<a href="print/c-jadwal?tipe=pertahun&tahun=<?= $tahun; ?>" target="_blank" class="btn btn-success">
																<i class="fas fa-print"></i>
															</a>
														<?php }else { ?>
															<a href="print/c-jadwal" target="_blank" class="btn btn-success">
																<i class="fas fa-print"></i>
															</a>
														<?php } ?>
												...
												<?php }else { ?>
													...
													<?php } ?>
													<a href="laporan-jadwal" class="btn btn-warning">
													...
													<table class="display table table-striped table-hover datatables" id="table-1">
														...
														<tbody>
															...
															if(isset($_GET['submit'])) {
																...
																if(isset($_GET['tipe'])) {
																	...
																	if($_GET['tipe'] == "pertanggal") {
																		...
																		$query = mysqli_query($conn, "SELECT * FROM tb_kelurahan, tb_pengaduan, tb_kecamatan, tb_bidang, tb_jadwal WHERE tb_kelurahan.fk_kecamatan=tb_kecamatan.id_kecamatan AND tb_pengaduan.id_kelurahan = tb_kelurahan.id_kelurahan AND tb_pengaduan.id_bidang = tb_bidang.id_bidang AND tb_jadwal.id_pengaduan = tb_pengaduan.id_pengaduan AND DATE(tanggal_jadwal) BETWEEN '$from' AND '$to' ORDER BY tanggal_jadwal ASC");
																	}else if($_GET['tipe'] == "perbulan") {
																		$bulan = $_GET['bulan'];
																		$tahun = $_GET['tahun'];
																		
																		$query = mysqli_query($conn, "SELECT * FROM tb_kelurahan, tb_pengaduan, tb_kecamatan, tb_bidang, tb_jadwal WHERE tb_kelurahan.fk_kecamatan=tb_kecamatan.id_kecamatan AND tb_pengaduan.id_kelurahan = tb_kelurahan.id_kelurahan AND tb_pengaduan.id_bidang = tb_bidang.id_bidang AND tb_jadwal.id_pengaduan = tb_pengaduan.id_pengaduan AND tanggal_jadwal LIKE '$tahun-$bulan%' ORDER BY tanggal_jadwal ASC");
																	}else if($_GET['tipe'] == "pertahun") {
																		$tahun = $_GET['tahun'];

																		$query = mysqli_query($conn, "SELECT * FROM tb_kelurahan, tb_pengaduan, tb_kecamatan, tb_bidang, tb_jadwal WHERE tb_kelurahan.fk_kecamatan=tb_kecamatan.id_kecamatan AND tb_pengaduan.id_kelurahan = tb_kelurahan.id_kelurahan AND tb_pengaduan.id_bidang = tb_bidang.id_bidang AND tb_jadwal.id_pengaduan = tb_pengaduan.id_pengaduan AND tanggal_jadwal LIKE '$tahun%' ORDER BY tanggal_jadwal ASC");
																	}else {
																		$query = mysqli_query($conn, "SELECT * FROM tb_kelurahan, tb_pengaduan, tb_kecamatan, tb_bidang, tb_jadwal WHERE tb_kelurahan.fk_kecamatan=tb_kecamatan.id_kecamatan AND tb_pengaduan.id_kelurahan = tb_kelurahan.id_kelurahan AND tb_pengaduan.id_bidang = tb_bidang.id_bidang AND tb_jadwal.id_pengaduan = tb_pengaduan.id_pengaduan ORDER BY tanggal_jadwal ASC");
																	}
															...
															while($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
																...
																<tr>
																	<td><?= $no++;?></td>
																	<td><?php echo formatDateIndonesia(date('d F Y', strtotime($row['tanggal_pengaduan'])));?> s/d <?php echo formatDateIndonesia(date('d F Y', strtotime($row['sampai_tanggal'])));?></td>
